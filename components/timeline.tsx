              ...
              {item.image && (
                <div className="w-full h-40 md:h-48 flex items-center justify-center p-6 bg-white/10">
                  <img
                    ...
                    className={`h-full w-auto object-contain ${item.image.includes('nsca') ? 'brightness-0 invert' : ''
                      }`}
                  />
                </div>
              )}
                ...
                <div className="flex items-start gap-3 mb-2">
                  ...
                  <div className="flex-1">
                    ...
                    {item.description && Array.isArray(item.description) && item.description.map((desc, idx) => (
                      <p
                        key={idx}
                        className="text-sm md:text-base text-muted-foreground leading-relaxed mb-2"
                      >
                        {desc}
                      </p>
                    ))}
